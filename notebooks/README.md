...
38. `37_colab_phase1_final_controls_metric_formula_revision_plan.ipynb`: record the claim-closed metric-formula revision decision boundary after metric-contract ambiguity, selecting no formula and authorizing no code/config change, threshold change, rerun or claim opening until a manual revision decision is made.
39. `38_colab_phase1_final_controls_metric_formula_decision.ipynb`: record the claim-closed manual metric-formula decision, with allowed outcomes limited to `raw_ba_ratio`, `gain_over_chance_ratio`, or `unresolved`, without rerunning controls, changing thresholds, editing metrics/logits, reclassifying artifacts or opening claims.
40. `39_colab_phase1_final_post_formula_decision_governance_update.ipynb`: link the final governance reconciliation state with the reviewed metric-formula decision, preserving unresolved formula-contract blockers and keeping all claims closed without rerunning analyses.
41. `40_colab_phase1_final_metric_formula_contract_remediation_plan.ipynb`: record the claim-closed remediation plan for the unresolved metric-formula contract, authorizing only future contract/docs/config proposal work and no formula selection, runtime change, threshold change, control rerun or claim opening.
42. Full real decoder phases only after locked prereg, required Phase 0.5 controls, Phase 1 readiness checks, Phase 1 smoke contract review pass, real model-smoke artifacts are reviewed, gap review blockers are resolved, final claim-package plan, final comparator artifact plan, final split/feature/leakage plan, final split manifest, final feature manifest, final leakage audit, final comparator runner readiness, final feature matrix, final comparator runner outputs, final A2d covariance/tangent outputs, final comparator reconciliation, final controls, final dedicated controls, final calibration, final influence, final reporting, final governance reconciliation, final claim-state closeout, final remediation plan, final controls remediation audit, final controls metric-contract audit, final controls metric-formula revision plan, final controls metric-formula decision, final post-formula-decision governance update, final metric-formula contract remediation plan and the required comparator/control/calibration/influence/reporting package are implemented and reviewed.
