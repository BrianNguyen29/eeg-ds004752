...
from src.phase1.a3_smoke import Phase1A3SmokeError, run_phase1_a3_smoke


class Phase1A3SmokeTests(unittest.TestCase):
    def test_a3_smoke_writes_non_claim_artifacts_and_distillation_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = run_phase1_a3_smoke(
                ...
                output_root=root / "phase1_a3_smoke",
                repo_root=Path.cwd(),
                max_outer_folds=2,
            ...
            self.assertTrue((result.output_dir / "a3_logits_smoke").exists())
            self.assertTrue((result.output_dir / "a3_metrics_smoke.json").exists())
            self.assertTrue((result.output_dir / "a3_distillation_audit.json").exists())
            self.assertTrue((result.output_dir / "a3_teacher_manifest.json").exists())
            self.assertTrue((result.output_dir / "a3_feature_manifest.json").exists())
            self.assertTrue((result.output_dir / "calibration_a3_smoke_report.json").exists())
            self.assertTrue((result.output_dir / "negative_controls_a3_smoke_report.json").exists())
            self.assertTrue((result.output_dir / "influence_a3_smoke_report.json").exists())

            summary = json.loads(result.summary_path.read_text(encoding="utf-8"))
            self.assertEqual(summary["status"], "phase1_a3_distillation_smoke_complete")
            self.assertEqual(summary["comparator"], "A3_distillation")
            ...
            self.assertFalse(summary["final_a3_comparator"])
            self.assertFalse(summary["claim_ready"])
            self.assertTrue(summary["does_not_estimate_privileged_transfer_efficacy"])

            audit = json.loads((result.output_dir / "a3_distillation_audit.json").read_text(encoding="utf-8"))
            self.assertFalse(audit["outer_test_subject_used_for_any_fit"])
            self.assertFalse(audit["real_ieeg_teacher_used"])
            self.assertFalse(audit["privileged_features_used"])
            for fold in audit["folds"]:
                self.assertTrue(fold["no_outer_test_subject_in_any_fit"])
                self.assertTrue(fold["no_outer_test_subject_in_normalization_fit"])
                self.assertTrue(fold["no_outer_test_subject_in_teacher_fit"])
                self.assertTrue(fold["no_outer_test_subject_in_teacher_outputs_for_student_fit"])
                ...
                self.assertNotIn(fold["outer_test_subject"], fold["teacher_fit_subjects"])
                self.assertNotIn(fold["outer_test_subject"], fold["teacher_output_subjects_for_student_fit"])
                self.assertNotIn(fold["outer_test_subject"], fold["student_fit_subjects"])

            teacher_manifest = json.loads((result.output_dir / "a3_teacher_manifest.json").read_text(encoding="utf-8"))
            self.assertEqual(teacher_manifest["status"], "a3_teacher_manifest_smoke")
            self.assertFalse(teacher_manifest["real_ieeg_teacher_used"])
            self.assertFalse(teacher_manifest["privileged_features_used"])
            self.assertFalse(teacher_manifest["teacher_used_at_inference"])

    def test_a3_smoke_requires_both_classes_per_fold(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            gate0 = root / "gate0" / "run"
            dataset = root / "data" / "ds004752"
            ...
            with self.assertRaises(Phase1A3SmokeError):
                run_phase1_a3_smoke(
                    prereg_bundle=prereg,
                    readiness_run=readiness,
                    dataset_root=dataset,
    ...
    def test_cli_phase1_help_exposes_a3_smoke_flag(self) -> None:
...
def _precomputed_feature_rows() -> dict[str, object]:
    ...
    for subject_index, subject in enumerate(["sub-01", "sub-02", "sub-03"], start=1):
        domain_shift = subject_index * 0.1
        for trial in range(8):
            label = trial % 2
            signal = 0.5 if label else -0.4
            rows.append(
                {
                    "subject": subject,
                    "session": "ses-01",
                    ...
                    "features": [
                        ...
                        -signal * 0.35 + domain_shift * 0.2,
                        subject_index * 0.04 + label * 0.05,
                        0.08 * trial,
                    ],
                }
            )
